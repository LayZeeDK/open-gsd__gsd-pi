...
`gsd_task_contract` reads back the planning contract `gsd_replan_task` demands for one task, so a single-field edit is no longer a read-modify-write with no read. `projectDir` is optional; `milestoneId`, `sliceId`, and `taskId` are required. It returns `title`, `description`, `estimate`, `files`, `verify`, `inputs`, and `expectedOutput` under the same names `gsd_replan_task` accepts, so untouched fields can be resent verbatim rather than reconstructed. It also reports `targetRepositories`, `fullPlanMdPresent`/`fullPlanMdLength`, and `unresendableFields`, with a `caveats` field explaining each. `targetRepositories` is not accepted by any `gsd_replan_task` tool schema, in-process or over MCP, so it cannot be resent; every replan re-resolves it from the parent slice's targets, then from targets derived from `files` and `expectedOutput`, then from the default -- so a task narrowed below its slice widens back and the narrowing is lost. Record the value before replanning and check it afterwards. No MCP schema carries `targetRepositories` at all, so over MCP a lost narrowing cannot be restored and should be reported rather than assumed repairable. `unresendableFields` names each field that cannot be resent verbatim and why: `title`, `description`, `estimate`, or `verify` stored empty (`gsd_replan_task` rejects a blank); a `verify` that names a GSD tool rather than a shell command (`gsd_plan_slice` and legacy imports do not apply that gate, but `gsd_replan_task` does); and a blank entry inside `files`, `inputs`, or `expectedOutput`, which makes `gsd_replan_task` reject the whole array. Every field it does not list is safe to resend exactly as returned. The response also carries `status`, `canonicalStatus`, and `sliceStatus`. `gsd_replan_task` refuses a closed task, a task whose canonical lifecycle row is completed or cancelled, a task in a closed slice, or a task whose slice row is missing -- each before it looks at the contract -- so a caveat names the specific blocker and points at `gsd_task_reopen` / `gsd_slice_reopen` rather than letting the write bounce. The legacy and canonical statuses are reported separately because `gsd_replan_task` checks both, and they can diverge. The authored plan body itself is not returned because it is unbounded and cannot be resent; a non-empty `fullPlanMd` means the rendered PLAN.md bypasses the per-task renderer and can be stale after a replan. A missing task is reported as an error rather than an empty contract a caller could write back as a wipe.
